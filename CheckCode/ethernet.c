...
int main(void)
{
    ...
    // test port list for TCP
    // for now we are pretending port 1883 is open so we can test the helper functions
    uint16_t testPorts[] = {1883};

    // clear packet buffer before use
    ...
    // load our TCP open port list
    // this means incoming TCP packets to port 1883 should be treated as open
    setTcpPortList(testPorts, 1);

    // Init ethernet interface (eth0)
    putsUart0("\r\nStarting eth0\n");
    initEther(ETHER_UNICAST | ETHER_BROADCAST | ETHER_HALFDUPLEX);
    setEtherMacAddress(2, 3, 4, 5, 6, 110);
    ...
    while (true)
    {
        ...
        if (isEtherDataAvailable())
        {
            ...
            if (isIp(data))
            {
                ...
                if (isIpUnicast(data))
                {
                    putsUart0("IP unicast to me\r\n");

                    // reply to ping requests
                    if (isPingRequest(data))
                    {
                        putsUart0("Ping request seen\r\n");
                    ...
                    if (isUdp(data))
                    {
                        putsUart0("UDP packet seen\r\n");
                    }
                }

                // UDP handling stays as-is
                if (isUdp(data))
                {
                    if (isDhcpResponse(data))
                    {
                        putsUart0("DHCP packet accept\r\n");
